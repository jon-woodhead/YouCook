...
//$link = mysqli_connect("localhost","j02tour_jon","WOZVM875","j02tour_YouCook") or die("Error " . mysqli_error($link));
    ...
    $keyword = trim(strip_tags(htmlentities($_REQUEST['keyword'])));
    	$keyword = str_replace ("'","''",$keyword);
    
     if(!isset($keyword)) { 
          $keyword = 'ThereIsNoKeywordSetSoreturnNothing';
     }
...
$keywordArray = array_unique($keywordArray);
 ...
 //print_r($keywordArray);
    ...
    $lastElement = end($keywordArray);
    foreach ($keywordArray as &$value) {
       $query .= 'title LIKE "%'.$value.'%" OR ingredients LIKE "%'.$value.'%" ';
